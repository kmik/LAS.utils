...
callDir="$PWD"
java -Xmx16g -XX:ParallelGCThreads=4 -XX:ConcGCThreads=4 -cp ".:$curDir/lib/*:$curDir/target/:$GDAL_JAVA_PATH/*" lasIndex $@ 2>&1 | tee $callDir/lasIndex_$current_datetime.log || [ ! -s $callDir/lasIndex_$current_datetime.log ] && rm $callDir/lasIndex_$current_datetime.log
