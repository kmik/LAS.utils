...
# Get the directory from which the script was called
callDir="$PWD"
stdbuf -oL java -Xmx32g -XX:ParallelGCThreads=8 -XX:ConcGCThreads=8 -cp ".:$curDir/lib/*:$curDir/target/:$GDAL_JAVA_PATH/*" retileRaster "$@" 2>&1 | tee "$callDir/retileRaster_$current_datetime.log"
