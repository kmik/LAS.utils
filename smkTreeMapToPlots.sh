...
callDir="$PWD"
java -Xmx16g -XX:ParallelGCThreads=4 -XX:ConcGCThreads=4 -cp ".:$curDir/lib/*:$curDir/target/:$GDAL_JAVA_PATH/*" treeMapToPlots $@ 2>&1 | tee $callDir/treeMapToPlots_$current_datetime.log || [ ! -s $callDir/treeMapToPlots_$current_datetime.log ] && rm $callDir/treeMapToPlots_$current_datetime.log
