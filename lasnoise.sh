...
callDir="$PWD"
java -Xmx16g -XX:ParallelGCThreads=4 -XX:ConcGCThreads=4 -cp ".:$curDir/lib/*:$curDir/target/:$GDAL_JAVA_PATH/*" lasnoise $@ 2>&1 | tee $callDir/lasnoise_$current_datetime.log || [ ! -s $callDir/lasnoise_$current_datetime.log ] && rm $callDir/lasnoise_$current_datetime.log
