...
callDir="$PWD"
java -Xmx16g -XX:ParallelGCThreads=4 -XX:ConcGCThreads=4 -cp ".:$curDir/lib/*:$curDir/target/:$GDAL_JAVA_PATH/*" lasExtent $@ 2>&1 | tee $callDir/lasExtent_$current_datetime.log || [ ! -s $callDir/lasExtent_$current_datetime.log ] && rm $callDir/lasExtent_$current_datetime.log
