...
#callDir="$PWD"
#java -Xmx16g -XX:+UseParallelGC -XX:ParallelGCThreads=8 -XX:ConcGCThreads=8 -cp ".:$curDir/lib/*:$curDir/target/:$GDAL_JAVA_PATH/*" las2dsm $@ 2>&1 | tee $callDir/las2dsm_$current_datetime.log || [ ! -s $callDir/las2dsm_$current_datetime.log ] && rm $callDir/las2dsm_$current_datetime.log
...
stdbuf -oL java -Xmx32g -XX:ParallelGCThreads=8 -XX:ConcGCThreads=8 -cp ".:$curDir/lib/*:$curDir/target/:$GDAL_JAVA_PATH/*" inpho2micmac "$@" 2>&1 | tee "$callDir/inpho2micmac_$current_datetime.log"
...
if [ $exit_status -eq 0 ]; then
  rm "$callDir/las2dsm_$current_datetime.log"
fi
