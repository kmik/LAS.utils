...
callDir="$PWD"
java -Xmx16g -XX:ParallelGCThreads=4 -XX:ConcGCThreads=4 -cp ".:$curDir/lib/*:$curDir/target/:$GDAL_JAVA_PATH/*" lasSplit $@ 2>&1 | tee $callDir/lasSplit_$current_datetime.log || [ ! -s $callDir/lasSplit_$current_datetime.log ] && rm $callDir/lasSplit_$current_datetime.log
