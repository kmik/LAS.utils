...
callDir="$PWD"
java -Xmx16g -XX:ParallelGCThreads=4 -XX:ConcGCThreads=4 -cp ".:$curDir/lib/*:$curDir/target/:$GDAL_JAVA_PATH/*" lasmerge $@ 2>&1 | tee $callDir/lasmerge_$current_datetime.log || [ ! -s $callDir/lasmerge_$current_datetime.log ] && rm $callDir/lasmerge_$current_datetime.log
