...
stdbuf -oL java -Xmx32g -XX:ParallelGCThreads=8 -XX:ConcGCThreads=8 -cp ".:$curDir/lib/*:$curDir/target/:$GDAL_JAVA_PATH/*" extractRaster "$@" 2>&1 | tee "$callDir/extractRaster_$current_datetime.log"
...
if [ $exit_status -eq 0 ]; then
  rm "$callDir/extractRaster_$current_datetime.log"
fi
