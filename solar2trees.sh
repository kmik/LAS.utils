...
callDir="$PWD"
java -Xmx16g -XX:ParallelGCThreads=4 -XX:ConcGCThreads=4 -cp ".:$curDir/lib/*:$curDir/target/:$GDAL_JAVA_PATH/*" solarFieldTrees $@ 2>&1 | tee $callDir/solarFieldTrees_$current_datetime.log || [ ! -s $callDir/solarFieldTrees_$current_datetime.log ] && rm $callDir/solarFieldTrees_$current_datetime.log
