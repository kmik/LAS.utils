...
public class stemDetector_filtered{
     ...
     * @param y_interval
     */
    public stemDetector_filtered(LASReader pointCloud, double y_interval, double z_cutoff, int coreNumber, argumentReader aR) throws IOException{
                ...
                for(int z = 1; z < this.zDim-1; z++) {
                    if(in[x][y][z] != null)
                        if (!in[x][y][z].isSurface) {
                            ...
                            if(sp.voxels.size() > min_n_voxels) { // && (ratio1 < 0.50 || ratio2 < 0.50)
...
*/
        ...
        for(int i = 0; i < n; i += 10000) {
            ...
            for (int j = 0; j < maxi; j++) {
                ...
                //KdTree.XYZPoint point = new KdTree.XYZPoint(tempPoint.x, tempPoint.y, tempPoint.z);
                //point.setIndex(i+j);
                //tree.add(point);
...
*/
        ...
        KdTree.XYZPoint searchThis = new KdTree.XYZPoint(0,0,0);
        ...
        for(int i = 0; i < n; i += 10000) {
            ...
            for (int j = 0; j < maxi; j++) {
                ...
                nearest = (List<KdTree.XYZPoint>)tree.nearestNeighbourSearch(2, searchThis);
...
*/
        ...
        int[] stemPointCounts_2 = new int[this.numberOfStems + 1];
        int[] stemPointCounts_buffer = new int[this.numberOfStems + 1];
        ...
        HashMap<Integer, ArrayList<ArrayList<double[]>>> trunkPoints = new HashMap<>();
        HashMap<Integer, Integer> trunkPoints_n = new HashMap<>();

        int debug_n_points_1 = 0;
        int debug_n_points_2 = 0;
        ...
        for(int i = 0; i < pointCloud.getNumberOfPointRecords(); i += 200000) {
             ...
             */
                ...
                if (tempPoint.z >= z_cutoff && tempPoint.z < z_cutoff_max) {// && tempPoint.pointSourceId > 0) {
                    ...
                    if (surface[xLocation][yLocation][zLocation] != null) {
                        if(!surface[xLocation][yLocation][zLocation].ignore_this)
                        if ( (surface[xLocation][yLocation][zLocation].isSurface || surface[xLocation][yLocation][zLocation].isSurface_buffer) && surface[xLocation][yLocation][zLocation].id >= 0) {
                            ...
                            stemPointCounts[surface[xLocation][yLocation][zLocation].id]++;
                            stemPointCounts_2[surface[xLocation][yLocation][zLocation].id]++;
                            debug_n_points_1++;

                        }else if(!surface[xLocation][yLocation][zLocation].isSurface_buffer){

                            /* Not a surface, so we check if it can be added to another close one */
                            int id = checkProximity(surface, xLocation, yLocation, zLocation);

                            if(id != -1){

                                //System.out.println(xLocation + " " + yLocation + " " + zLocation);
                                //System.out.println("MERGED! " + surface[xLocation][yLocation][zLocation].isSurface_buffer);
                                //System.out.println("MERGED! " + surface[xLocation][yLocation][zLocation].id);
                                stemPointCounts[id]++;
                                stemPointCounts_2[id]++;
                                debug_n_points_1++;


                            }else
                                surface[xLocation][yLocation][zLocation].ignore_this = true;
                        }
        ...
        boolean[] goodPoints = new boolean[(int)pointCloud.getNumberOfPointRecords()];

        double hcs_size = 0.3;

        int number_of_hcs =  (int)Math.ceil((z_cutoff_max - z_cutoff) / hcs_size);

        RANSAC_circleFitter RcF = new RANSAC_circleFitter();
        double ransac_distance_threshold = 0.025;

        RcF.setMaxRadius(0.20);
        RcF.setMinRadius(0.0);
        HashSet<Integer> badTrunkBasedOnRansac = new HashSet<>();
...
*/
                ...
                if (tempPoint.z >= z_cutoff && tempPoint.z < z_cutoff_max) {// && tempPoint.pointSourceId > 0) {
                    ...
                    if (surface[xLocation][yLocation][zLocation] != null) {
                        ...
                        if ( (surface[xLocation][yLocation][zLocation].isSurface || surface[xLocation][yLocation][zLocation].isSurface_buffer) && surface[xLocation][yLocation][zLocation].id >= 0) {
                            ...
                            if (!trunkPoints.containsKey(surface[xLocation][yLocation][zLocation].id)) {

                                trunkPoints.put(surface[xLocation][yLocation][zLocation].id, new ArrayList<>(number_of_hcs));
                                trunkPoints_n.put(surface[xLocation][yLocation][zLocation].id, 1);

                                for(int c = 0; c < number_of_hcs; c++){
                                    trunkPoints.get(surface[xLocation][yLocation][zLocation].id).add(new ArrayList());
                                }

                                int bin = (int)((tempPoint.z - z_cutoff) / hcs_size);
                                trunkPoints.get(surface[xLocation][yLocation][zLocation].id).get(bin).add(new double[]{tempPoint.x, tempPoint.y, tempPoint.z, i});
                            } else {
                                int bin = (int)((tempPoint.z - z_cutoff) / hcs_size);
                                trunkPoints.get(surface[xLocation][yLocation][zLocation].id).get(bin).add(new double[]{tempPoint.x, tempPoint.y, tempPoint.z, i});
                                trunkPoints_n.put(surface[xLocation][yLocation][zLocation].id, trunkPoints_n.get(surface[xLocation][yLocation][zLocation].id) + 1);
                            }

                            stemPointCounts_2[surface[xLocation][yLocation][zLocation].id]--;
                            debug_n_points_2++;
                            if(stemPointCounts_2[surface[xLocation][yLocation][zLocation].id] < 0)
                                throw new toolException("Something went wrong.");
                            //if (trunkPoints_n.get(surface[xLocation][yLocation][zLocation].id) == stemPointCounts[surface[xLocation][yLocation][zLocation].id]) {
                            if(stemPointCounts_2[surface[xLocation][yLocation][zLocation].id] == 0){

                                System.out.println("PROCESS TRUNK!! " + surface[xLocation][yLocation][zLocation].id);

                                int number_of_good_slices = 0;

                                for(int i_ = 0; i_ < trunkPoints.get(surface[xLocation][yLocation][zLocation].id).size(); i_++){
                                    if(trunkPoints.get(surface[xLocation][yLocation][zLocation].id).get(i_).size() > 5) {
                                        number_of_good_slices++;
                                    }
                                }

                                double good_length_in_meters = number_of_good_slices * hcs_size;

                                if(good_length_in_meters >= 1.0) {

                                    int goodRansacs = 0;
                                    int maxConsecutiveGoodRansacs = 0;

                                    for (int i_ = 0; i_ < trunkPoints.get(surface[xLocation][yLocation][zLocation].id).size(); i_++) {


                                        if (trunkPoints.get(surface[xLocation][yLocation][zLocation].id).get(i_).size() > 5) {
                                            int d = (int) Math.max(1, (trunkPoints.get(surface[xLocation][yLocation][zLocation].id).get(i_).size()) * 0.5);
                                            RcF.initialize5(trunkPoints.get(surface[xLocation][yLocation][zLocation].id).get(i_), 3, d, ransac_distance_threshold, 1000, 1, aR.set_seed);
                                            //System.out.println("RADIUS: " + RcF.radius_ransac + " " + RcF.ransacFailed);

                                            if (!RcF.ransacFailed) {

                                                for (double[] d_ : RcF.ransac_inlier_points_a) {

                                                    //System.out.println(Arrays.toString(d_));
                                                    goodPoints[(int) d_[3]] = true;

                                                }

                                                goodRansacs++;

                                                if(goodRansacs > maxConsecutiveGoodRansacs)
                                                    maxConsecutiveGoodRansacs = goodRansacs;
                                                //System.out.println("---------------------");
                                    ...
                                    if(maxConsecutiveGoodRansacs < (1.0 / hcs_size)){
                                        badTrunkBasedOnRansac.add(surface[xLocation][yLocation][zLocation].id);
                                    }
                                }
        ...
        //System.out.println(debug_n_points_1 + " == " + debug_n_points_2);
        //System.exit(1);
        ...
        /* TrunkPOints contain all the points from a single trunk */
            ...
            if(tempPoint.classification != 2) {
                ...
                if (zLocation < this.zDim) {
                    ...
                    if (surface[xLocation][yLocation][zLocation] != null) {
                        ...
                        if (surface[xLocation][yLocation][zLocation].isSurface && surface[xLocation][yLocation][zLocation].id >= 0 && !badTrunkBasedOnRansac.contains(surface[xLocation][yLocation][zLocation].id)  //&& !badStems.contains((short) surface[xLocation][yLocation][zLocation].id
                                && goodPoints[i]) {
...
*/
    ...
    public int checkProximity(VoxelNeighborhood[][][] surface, int x_, int y_, int z_){

        int buffer_size = 1;

        HashSet<Integer> surfaces = new HashSet<>();
        int id = -1;


        for(int x = x_ - buffer_size; x <= x_ + buffer_size; x++)
            for(int y = y_ - buffer_size; y <= y_ + buffer_size; y++)
                for(int z = z_ - buffer_size * 2; z <= z_ + buffer_size * 2; z++)
                    if(z > 0 && x > 0 && y > 0 && z < zDim && y < yDim && x < xDim)
                        if(surface[x][y][z] != null)
                            if(surface[x][y][z].isSurface && surface[x][y][z].id >= 0){

                                surfaces.add(surface[x][y][z].id);
                                id = surface[x][y][z].id;
                                //surface[x_][y_][z_].addBufferNeighbor(x, y, z);



                            }

        //System.out.println("Number of unique ids: " + surfaces.size() + " id: " + id);

                            if(surfaces.size() == 1) {
                                surface[x_][y_][z_].isSurface_buffer = true;
                                surface[x_][y_][z_].id = id;
                                //stemPointCounts[surface[x_][y_][z_].id]++;
                                return id;
                            }
                            else
                                return -1;
