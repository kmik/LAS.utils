...
public class LASwrite {
	...
	*/
	...
	/**
	*	Update header without priori knowledge about the point data
	*
	*	@param to 				Point cloud, which header is to be checked
	*	@param pointCount2 		(Deprecated)	
	*
	* 	@author  Kukkonen Mikko
	* 	@version 0.1
	* 	@since 09.03.2018  
	*/
	...
	public static void updateHeader2(LASraf to, int pointCount2) throws IOException{

		long n = pointCount2;
			to.writePointCount(pointCount2);

		LASReader file = new LASReader(to.file);
		...
		to.writeMinMax(minX, maxX, minY, maxY, maxZ, minZ);
		to.writePByReturn(pointsByReturn);
	...
	*/
	...
	public static synchronized void copyLAS(LASReader from, LASraf to, PointInclusionRule rule, String softwareName, String otype, String oparse) throws IOException{
		...
		if(otype.equals("las")){
			...
			to.writeAscii(4, from.fileSignature);
			//to.fileChannel.position(to.raFile.length());
		   ...
		   // to.writeUnsignedByte((byte)from.versionMajor);// = braf.readUnsignedByte();
		    ...
		    //to.writeUnsignedShort((short)from.headerSize);// = braf.readUnsignedShort();
		    //System.out.println((int)from.headerSize);
		    
		    //to.writeUnsignedInt((int)from.offsetToPointData);// = braf.readUnsignedInt();
		    //to.writeUnsignedInt((int)from.offsetToPointData); //227
		    ...
		    to.writeUnsignedShort((short)from.pointDataRecordLength);// = braf.readUnsignedShort();
		    
		    to.writeUnsignedInt((int)from.legacyNumberOfPointRecords);// = braf.readUnsignedInt();
		    //System.out.println(from.legacyNumberOfPointRecords);
		    //legacyNumberOfPointsByReturn = new long[5];
		   	to.writeUnsignedInt((int)from.legacyNumberOfPointsByReturn[0]);

		   	//System.out.println(from.legacyNumberOfPointsByReturn[1]);
		   	
		   	to.writeUnsignedInt((int)from.legacyNumberOfPointsByReturn[1]);
		   	to.writeUnsignedInt((int)from.legacyNumberOfPointsByReturn[2]);
		   	to.writeUnsignedInt((int)from.legacyNumberOfPointsByReturn[3]);
		   	to.writeUnsignedInt((int)from.legacyNumberOfPointsByReturn[4]);
		    ...
		    to.writeDouble(from.maxX);// = braf.readDouble();
		    to.writeDouble(from.minX);// = braf.readDouble();
		    to.writeDouble(from.maxY);// = braf.readDouble();
		    to.writeDouble(from.minY);// = braf.readDouble();
		    to.writeDouble(from.maxZ);// = braf.readDouble();
		    to.writeDouble(from.minZ);// = braf.readDouble();
			...
			*/
		    ...
		    long n = from.getNumberOfPointRecords();
			LasPoint tempPoint = new LasPoint();
			//to.writeBuffer();
			//to.writeShort((short)0);
			...
			for(int i = 0; i < n; i++){
				...
				from.readRecord(i, tempPoint);
				//System.out.println(tempPoint.userData);
				if(rule.ask(tempPoint, i, true)){
					pointCount++;
					
					int tempIndex = tempPoint.returnNumber - 1;
					if(tempIndex < 0)
						tempIndex = 0;

				 	pointsByReturn[tempIndex]++;
					...
					if(y > maxY)
						maxY = y;
					//System.out.println((int)((x - from.xOffset) / from.xScaleFactor));
					int lx = (int)((x - from.xOffset) / from.xScaleFactor);
		    		int ly = (int)((y - from.yOffset) / from.yScaleFactor);
		    		int lz = (int)((z - from.zOffset) / from.zScaleFactor);
					...
					*/
					...
					//System.out.println(getBit(myByte, 2));
					...
					//System.out.println("-----------");
					//System.out.println(Integer.toBinaryString((int)myByte));
					//System.out.println(bitti);
			...
			//System.out.println(Arrays.toString(pointsByReturn));
			to.writePointCount(1000);
			LASReader testi = new LASReader(to.file);
			//pointsByReturn[0] = 99;
			//System.out.println(testi.legacyNumberOfPointRecords == pointCount);
			//System.out.println(Arrays.equals(pointsByReturn, testi.legacyNumberOfPointsByReturn));
			//to.fileOuputStream.close();

			if(pointCount != testi.legacyNumberOfPointRecords)
				updateHeader(pointCount, pointsByReturn, minX, maxX, minY, maxY
				, maxZ, minZ, to);
			if(!Arrays.equals(pointsByReturn, testi.legacyNumberOfPointsByReturn))
				updateHeader(pointCount, pointsByReturn, minX, maxX, minY, maxY
				, maxZ, minZ, to);
			if(minX != testi.minX || maxX != testi.maxX || minY != testi.minY || maxY != testi.maxY ||
				minZ != testi.minZ || maxZ != testi.maxZ)
				updateHeader(pointCount, pointsByReturn, minX, maxX, minY, maxY
				, maxZ, minZ, to);
		}
		...
		if(otype == "txt"){
			...
			String pathi = to.getFileReference().getAbsolutePath().split(".las")[0] + ".txt";
			to.getFileReference().delete();

			File ofile = new File(pathi);
			...
			try{
				...
				for(int i = 0; i < n; i++){
					...
					if(rule.ask(tempPoint, i, true)){
						...
						String out = LASpoint2String(tempPoint, oparse);
