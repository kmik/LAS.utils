...
#callDir="$PWD"
#java -Xmx32g -XX:ParallelGCThreads=4 -XX:ConcGCThreads=4 -cp ".:$curDir/lib/*:$curDir/target/:$GDAL_JAVA_PATH/*" lasAlign $@ 2>&1 | tee $callDir/lasAlign_$current_datetime.log || [ ! -s $callDir/lasAlign_$current_datetime.log ] && rm $callDir/lasAlign_$current_datetime.log
...
# Get the directory from which the script was called
callDir="$PWD"
stdbuf -oL java -Xmx32g -XX:ParallelGCThreads=8 -XX:ConcGCThreads=8 -cp ".:$curDir/lib/*:$curDir/target/:$GDAL_JAVA_PATH/*" lasAlign "$@" 2>&1 | tee "$callDir/lasAlign_$current_datetime.log"
...
if [ $exit_status -eq 0 ]; then
  rm "$callDir/lasAlign_$current_datetime.log"
fi
