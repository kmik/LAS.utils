...
callDir="$PWD"
java -Xmx16g -XX:ParallelGCThreads=4 -XX:ConcGCThreads=4 -cp ".:$curDir/lib/*:$curDir/target/:$GDAL_JAVA_PATH/*" parseInpho $@ 2>&1 | tee $callDir/parseInpho_$current_datetime.log || [ ! -s $callDir/parseInpho_$current_datetime.log ] && rm $callDir/parseInpho_$current_datetime.log
